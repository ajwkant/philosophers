...
echo -e '\x1B[91m _____ _   _ _                 _                  _____        _
...
if [ "$#" -ne 2 ]; then
    ...
    echo -e "\tType 0: test philo_one, philo_two and philo_three"
    echo -e "\tType 1: test philo_one only"
    echo -e "\tType 2: test philo_two only"
    echo -e "\tType 3: test philo_three only"
    exit
fi
...
echo -e "\x1B[92m[+] Given Folder: $1"
echo -e "[+] Test Type: $2\x1B[0m\n"
echo -e "\x1B[94m[+] In Case of a failed test, please check ./errors_log file for more information\x1B[0m\n"
...
test_one ()
{
    ...
    if [ $output = "died" ];then
        echo -e "\x1B[92m[+] Test #1 Succeeded !\x1B[0m"
    else
        echo -e "\x1B[91m[+] Test #1 Failed !\x1B[0m"
...
test_two ()
{
    echo -e "\x1B[94m[+] Test #2: Executing your program for 180 second, please wait...\x1B[0m"
    ...
    while [ $i -lt 180 ];do
        ...
        pgrep $1 > /dev/null
        if [ "$?" -ne 0 ];then
            echo -e "\r\x1B[91m[+] Test #2 Failed\x1B[0m"
    ...
    if [ $error -eq 0 ];then
        pkill $1
        echo -e "\r\x1B[92m[+] Test #2 Succeeded\x1B[0m"
    fi
}

test_three ()
{
    echo -e "\x1B[94m[+] Test #3: Executing your program for 180 second, please wait...\x1B[0m"
    ...
    while [ $i -lt 180 ];do
        ...
        pgrep $1 > /dev/null
        if [ "$?" -ne 0 ];then
            echo -e "\r\x1B[91m[+] Test #3 Failed\x1B[0m"
    ...
    if [ $error -eq 0 ];then
        pkill $1
        echo -e "\r\x1B[92m[+] Test #3 Succeeded\x1B[0m"
    fi
}
...
test_four ()
{
    ...
    if [ "$?" -eq 1 ];then
        ...
        if [ $lines -ge $4 ];then
            echo -e "\t\x1B[92m[+] Test #4-$5 Succeeded\x1B[0m"
        else
            echo -e "\t\x1B[91m[+] Test #4-$5 Failed\x1B[0m"
...
test_five ()
{
    echo -e "\x1B[94m[+] Test #5 on progress, please wait...\x1B[0m"
    i=1
    t=0
    error=0
    while [ $i -le 10 ];do
        ...
        if [ "$?" -eq 1 ];then
            ...
            if [ $i -gt 1 ];then
                ...
                x=${x#-}
                if [ $x -gt 10 ];then
                    printf "\r\x1B[91m[+] Test #5 Failed\x1B[0m\n"
    ...
    if [ $error -eq 0 ];then
        echo -e "\r\x1B[92m[+] Test #5 Succeeded\x1B[0m"
    fi
}
...
test_six ()
{
    ...
    forks=$(pgrep $1 | wc -l)
    if [ "$forks" -eq 11 ];then
        printf "\r\x1B[92m[+] Test #6 Succeeded\x1B[0m\n"
    else
        printf "\r\x1B[91m[+] Test #6 Failed\x1B[0m\n"
...
if [ "$2" -eq 1 -o "$2" -eq 0 ];then
    ...
    echo -e "\x1B[94m[+] Test #4 on progress, please wait...\x1B[0m"
